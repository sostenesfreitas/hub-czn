...
def _build_library() -> list[CardEntry]:
    ...
    for card_id, card in cards.items():
        ...
        match = _CHAR_RE.match(card_id)
        char_res_id = int(match.group(1)) if match else None

        total_eff = 0
        total_hits = 0
        seen_types: list[str] = []

        for effect_id in card.get("link_skill_eff_id", []):
            effect = effects.get(effect_id, {})
            effect_type = effect.get("eff", "")
            label = _EFFECT_LABELS.get(effect_type)

            if label and label not in seen_types:
                seen_types.append(label)

            if effect_type == "SKILL_EFF_DMG":
                total_eff += effect.get("eff_value", 0)
                total_hits += effect.get("eff_count_value", 0)
    ...
    result.sort(key=lambda card: (card.char_res_id or 0, card.card_id))
    return result
...
def _get_character_metadata(char_res_id: int) -> dict:
    character = CHARACTERS.get(char_res_id) or {}
...
def get_cards(char_res_id: int | None = None):
    ...
    if char_res_id is not None:
        return [card for card in library if card.char_res_id == char_res_id]
...
def get_card_characters():
    """Return characters that have cards in the library, including class metadata."""
    db = game_db.get()
    names = db.get("names", {})
    library = _get_library()
    ...
    for card in library:
        if not card.char_res_id or card.char_res_id in seen:
            continue

        metadata = _get_character_metadata(card.char_res_id)

        seen[card.char_res_id] = {
            "char_res_id": card.char_res_id,
            "name": names.get(
                f"char_base@name@{card.char_res_id}",
                str(card.char_res_id),
            ),
    ...
    return [
        seen[char_res_id]
        for char_res_id in sorted(
            seen.keys(),
            key=lambda key: seen[key].get("name") or str(key),
        )
    ]