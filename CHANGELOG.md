...
### Fixed
- **Configuração: detecção de administrador** — substituída `IsUserAnAdmin()` (deprecated) por verificação direta do token de elevação (`TokenElevation`), que retornava falso negativo no sidecar PyInstaller mesmo ao executar como administrador
